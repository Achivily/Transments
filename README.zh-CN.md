...
- 来源选择：选择单张或多张图片、扫描文件夹、拖入文件或文件夹、按输入格式筛选、包含子文件夹。
- 输出设置与预设：应用转换预设、选择目标格式、优先无损编码、控制覆盖策略、指定输出文件夹。
- 批量队列：查看文件、路径和每个文件的转换状态。
- 结果面板：查看完成摘要、失败文件，并可立即打开输出目录。
- 操作栏：跟踪进度、取消任务、开始转换。
...
- **双语界面**：可在软件内切换英文和中文。
- **拖拽导入**：可将图片或文件夹拖入队列区域，并沿用当前筛选和递归扫描设置。
- **转换预设**：内置 WebP 归档、JPEG 分享、HEIC 转 PNG、AVIF 批量等一键工作流。
- **转换后结果面板**：查看成功/失败数量、失败文件名，并可直接打开输出文件夹。
...
2. 需要更快导入时，可直接把图片或文件夹拖入队列区域。
3. 使用 **扫描筛选** 指定源格式，例如 PNG 或 HEIC。
4. 应用转换预设，或在 **输出** 区手动选择目标格式。
5. 如需递归处理目录，开启 **包含子文件夹**。
6. 点击 **选择输出文件夹** 指定转换结果的保存位置。
7. 点击 **开始转换**，在队列中查看转换进度。
8. 在 **转换结果** 面板查看摘要，并按需直接打开输出文件夹。
9. 点击右上角语言/设置按钮，可在英文和中文之间切换。
...
- 预设卡片让常用工作流保持可见，不需要额外弹窗。
- 拖拽反馈直接显示在队列区域，也就是文件将要进入的位置。
...
- `scanFolder()` 按源格式扫描目录。
- `resolvePaths()` 将拖入的文件和文件夹整理为去重后的转换队列。
- `start()` 调度批量转换任务。
- `convertOne()` 处理单个文件转换。
